...
private:
    //Functions
    cv::Mat preprocessImage(const cv::Mat& frame, cv::Mat& out_color, Features& features);
    ...
    std::unique_ptr<FeatureDetector> featureDetector_;
...
template<typename Scalar, int CS>
cv::Mat SlamSystem<Scalar, CS>::preprocessImage(const cv::Mat &frame, cv::Mat &out_color, Features &features) {
    ...
    cv::drawKeypoints(keyImg, features.keypoints, keyImg, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
